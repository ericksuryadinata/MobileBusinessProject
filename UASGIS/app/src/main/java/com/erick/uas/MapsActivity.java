...
import com.erick.uas.Command.coomand;
import com.erick.uas.model.Lokasi;
import com.example.makichanz.uas.R;
import com.erick.uas.adapter.MyAdapter;
import com.erick.uas.settingDirection.GetDirectionsData;
import com.erick.uas.settingDirection.GetDistanceData;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, LocationListener, EasyPermissions.PermissionCallbacks, EasyPermissions.RationaleCallbacks {
    ...
    private Marker marker, marker_destination, marker_simpan;
...
//                  mMap.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds.build(), padding));
    ...
    public void makeDistance(TextView jarak, TextView price){
        Object[] data_transfer = new Object[4];
        String url = getDirectionsUrl();
        GetDistanceData getDistanceData = new GetDistanceData();
        data_transfer[0] = mMap;
        data_transfer[1] = url;
        data_transfer[2] = jarak;
        data_transfer[3] = price;
            ...
            public void onItemClick(MenuItem item) {
                        ...
                        public void onClick(View v) {
                            ...
                            mMap.setOnMapClickListener(new GoogleMap.OnMapClickListener() {
                                ...
                                public void onMapClick(final LatLng latLng) {
                                    ...
                                    marker_simpan = mMap.addMarker(new MarkerOptions().position(latLng).title(String.valueOf(latLng.latitude) +" "+ String.valueOf(latLng.longitude+" L")));
                                    new coomand().dialog_add_lokasi(MapsActivity.this, latLng.latitude, latLng.longitude,new coomand.Response_dialog_add() {
                                        @Override
                                        public void sukses(String nama) {
                                            dialog.show();
                                            new coomand().insertData(nama,latLng, new coomand.Response_insert() {
                                                @Override
                                                public void sukses() {
                                                    dialog.dismiss();
                                                    marker_simpan.remove();
                                                    Toast.makeText(MapsActivity.this,"Berhasil menambah data"+my_location.latitude+" "+my_location.longitude,Toast.LENGTH_SHORT);
                ...
                if(item.getTitle().equals("Make_route")){
                    if(ed_destination.getText().equals("")){
                        ...
                        final Dialog dialog_jarak = new Dialog(MapsActivity.this, R.style.Theme_Dialog);
                        dialog_jarak.requestWindowFeature(Window.FEATURE_NO_TITLE);
                        dialog_jarak.setContentView(R.layout.dialog_jarak);
                        dialog_jarak.show();
                        TextView harga,jarak;
                        harga= (TextView)dialog_jarak.findViewById(R.id.distance_dialog);
                        jarak= (TextView)dialog_jarak.findViewById(R.id.harga_dialog);
                        makeDirection();
                        makeDistance(jarak,harga);
